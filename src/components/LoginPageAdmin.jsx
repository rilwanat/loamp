...
export default function LoginPageAdmin({ isMobile }) {
  ...
  const handleAdminSignin = async (e) => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        setCookie("loamp-admin-token", token, expirationDays);
        setCookie("loamp-admin-details", JSON.stringify(response.data.adminData));
        ...
        openNotificationModal(false, "Admin Login Error", "Login Failed");
      }
    } catch (error) {
      setIsSigninLoading(false);
      // alert(error);
      if (
        ...
        setErrorMessage({ message: errorMessage });
        openNotificationModal(false, "Admin Login Error", errorMessage);
      } else if (
        error.response &&
        error.response.data &&
        ...
        setErrorMessage({
          message: "Admin Login failed. Please check your credentials and try again.",
        });
        openNotificationModal(
          false,
          "Admin Login Error",

...
export default function CreateMembershipPage({ isMobile }) {
  ...
  const handleSignup = async (e) => {
    ...
    if (
      ...
      openNotificationModal(false, "African Ambassadors Registration", "Registration Failed: Please enter valid credentials.");
    ...
    if (!isValidEmail(email)) {
      ...
      openNotificationModal(false, "African Ambassadors Registration", "Please, enter a valid email.");
      return;
    }

    if (!isValidPassword(password)) {
      setErrorMessage({ message: "Password must be at least 8 characters." });
      openNotificationModal(false, "African Ambassadors Registration", "Password must be at least 8 characters.");
      return;
    }

    if (!passwordsMatch(password, confirmPassword)) {
      setErrorMessage({ message: "Passwords must match." });
      openNotificationModal(false, "African Ambassadors Registration", "Passwords must match.");
      return;
    }
    ...
    try {
      ...
      if (response.data.status) {
        ...
        openNotificationModal(
          true,
          "African Ambassadors Registration",

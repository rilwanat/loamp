...
export default function LoginPageSuperAdmin({ isMobile }) {
    ...
    try {
      ...
      const response = await axiosInstance.post(
        (import.meta.env.VITE_IS_LIVE === "true"
          ? import.meta.env.VITE_API_SERVER_URL
          : import.meta.env.VITE_API_DEMO_SERVER_URL) +
          import.meta.env.VITE_SUPER_ADMIN_LOGIN,
        requestData,
        {
          headers: {
      ...
      if (response.data.status) {
        ...
        setCookie("loamp-super-admin-token", token, expirationDays);
        setCookie("loamp-super-admin-details", JSON.stringify(response.data.superAdminData));
        ...
        openNotificationModal(false, "Super-Admin Login Error", "Login Failed");
      }
    } catch (error) {
      setIsSigninLoading(false);
      // alert(error);
      if (
        ...
        setErrorMessage({ message: errorMessage });
        openNotificationModal(false, "Super-Admin Login Error", errorMessage);
      } else if (
        error.response &&
        error.response.data &&
        ...
        setErrorMessage({
          message: "Super-Admin Login failed. Please check your credentials and try again.",
        });
        openNotificationModal(
          false,
          "Super-Admin Login Error",
          "Super-Admin Login failed. Please check your credentials and try again."
        );
      }
    }
  };
                      ...
                      >
                        <div>
                          <h2 className="text-xl text-black font-bold">
                            Super Admin Login to the League of{" "}
